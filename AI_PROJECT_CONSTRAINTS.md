...
当前系统已经支持“打开页面自动触发有效调用”，不要再回退成纯展示型首页。

真实逻辑：

- 前端通过 `/api/workflows/status` 获取 `recommended_action`；
- 当值为 `run_daily_open_workflow` 时，首页会调用 `POST /api/workflows/daily-open`；
- 自动触发受 cooldown、正在运行状态、`auto_run_ai_on_open` 等配置约束；
- 自动流程不是每次打开都强制执行，60 分钟冷却期内允许跳过。

因此：

- 不能再声称“首页不会产生真实调用”；
- 也不能把页面打开即自动调用，误改成无限重复触发；
- 与自动流程相关的修改，必须同时检查前端触发条件和后端 cooldown 逻辑。
...
赛后评分以"开赛前用户可见的最后一份有效预测"为准，不再以 T-30 为核心口径。

用户真实使用场景是在北京时间白天查看今晚/凌晨比赛预测。赛后复盘应使用开赛前用户可见的最后一份有效预测快照。

T-30 如保留字段，只作为兼容或辅助信息，不作为核心评分依据。
...
不再使用 `no_locked_snapshot` 作为排除原因。只要存在开球前快照即可参与评分，无需 T-30 锁定。
...
Team Profile 已经接入当前代码，但当前历史数据模式是 `seed_mock_v1`。

这意味着：

- 结构、接口、评分、抽屉展示可以验证；
- 但历史画像本身暂时不是公开真实历史数据；
- 任何页面、接口、文档、报告都必须明确标注这一点。

### 7.2 Baseline 隔离

Team Profile 只能作为独立候选版本存在，不能污染 baseline。

当前要求：

- profile 存独立表；
- profile prediction 存独立表；
- profile 评分独立计算；
- baseline 行为保持原样。
...
- 4 个 AI 模型，`ensemble_weight` 分别为 0.33, 0.67, 0.67, 0.33，总和 = 2.0
- AI 总权重 = 0.30

4 个模型全部参与时，每个模型的 AI 内部占比：

- `0.33/2.0 = 0.165`，`0.67/2.0 = 0.335`，`0.67/2.0 = 0.335`，`0.33/2.0 = 0.165`
- 最终权重 = 上述占比 × 0.30

最终总权重：system(0.50) + market(0.20) + AI(0.30) = 1.00，归一化后不变。
...
daily-open 工作流应自动为符合条件的比赛补跑 AI 预测：
