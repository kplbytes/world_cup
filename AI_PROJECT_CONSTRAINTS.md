...
- 当前 AI prompt builder 仍把 Team Profile 标成 `disabled_display_only`，不要在文档或页面里把它写成“AI 直接使用画像作为输入”；
...
冠军与赛程页中的淘汰赛对阵卡，如果后端已返回 `id`，也必须复用同一个 `MatchDetailDrawer`，并通过 `/api/matches/{id}` 获取详情；不要再维护第二套对阵详情结构。

### 9.3 模型复盘状态边界

模型复盘页允许部分数据源失败，但不允许因为某个次级接口失败就整页一直停留在加载状态。
...
- 暂无已完赛淘汰赛样本，暂时不能下效果结论；
- 未来 48 小时真实淘汰赛仍缺 AI；
- 自动 AI workflow 当前是否启用。
...
- 如果对阵已具备真实 `match_id`，前端应允许从淘汰赛卡片继续钻取到共享比赛详情，而不是把淘汰赛页做成只读静态展示。
