...
| 定时任务 | APScheduler | 后台调度，自动适应比赛日节奏 |
...
```
启动 → 初始化数据库 → 种子数据 → 首次重算 → 修复锁定 → 启动调度器 → 就绪
                                                              │
                                              ┌───────────────┤
                                              │               │
                                        定时刷新          定时锁定
...
2. **`initialize_database()`**：建表、种子数据、首次重算、修复锁定
3. **调度器**：两个定时任务——数据刷新和快照锁定
...
| 数据获取 | TanStack Query v5 | 缓存、重试、后台刷新 |
...
AI 预测 (ai/service) ── 多模型独立预测
    │                      │
    │                      ├── DeepSeek V4 Flash/Pro (v1+v2)
    │                      ├── 小米 MiMo V2/V2.5 Pro (v1+v2)
    │                      └── 去重 + 抄袭检测
    │
    ▼
集成融合 (ensemble) ─── 加权融合
