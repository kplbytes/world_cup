...
- 会影响 baseline / numerical 赛前预测，但当前 AI prompt 仍把画像标记为 `disabled_display_only`；
...
1. **主预测链路**：画像会通过 `backend/app/prediction/profile_adapter.py` 转成 `MatchContext` 调整项，进入 baseline / numerical 重算，影响主预测概率与 xG；
2. **AI 链路**：当前 `backend/app/ai/prompt_builder.py` 仍把 Team Profile 作为 `disabled_display_only`，不直接把结构化画像调整注入 AI prompt；
3. **集成链路**：画像不是独立的第四个 Ensemble source，不单独占一个权重槽；它的影响体现在 baseline 家族输出里。

用户侧主要通过以下 API 查看画像数据：
...
- 仓库仍保留 `team_profile_predictions` 表和 `/api/team-profiles/evaluation` 端点用于模型复盘/历史评估；
- 这套独立评估不代表画像是独立第四预测源，也不改变上面“主预测进入 MatchContext、AI prompt 仍 display-only”的边界；
- 如果当前没有满足条件的独立评估样本，模型复盘页可能返回 `sample_count=0`，这属于数据状态，不应被文档写成“功能不存在”。
