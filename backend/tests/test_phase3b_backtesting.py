...
class TestNextMatchFrozenTime:
    """Test next_match logic with frozen times."""

    def _make_matches(self):
        """Create mock match objects for testing."""
        from dataclasses import dataclass
        @dataclass
        class MockMatch:
            id: str
            ...
            away_team_id: str = "B"
        return [
            MockMatch("esp-cpv", "scheduled", datetime(2026, 6, 16, 0, 0, tzinfo=timezone.utc)),
            MockMatch("bel-egy", "scheduled", datetime(2026, 6, 16, 3, 0, tzinfo=timezone.utc)),
            MockMatch("sau-uru", "scheduled", datetime(2026, 6, 16, 6, 0, tzinfo=timezone.utc)),
        ]

    def test_before_midnight_next_match_is_spain(self):
        """At 21:48 UTC+8 (13:48 UTC), next match is Spain vs Cape Verde."""
        matches = self._make_matches()
        now_utc = datetime(2026, 6, 15, 13, 48, tzinfo=timezone.utc)
        scheduled = [m for m in matches if m.status == "scheduled" and m.kickoff > now_utc]
        scheduled.sort(key=lambda m: (m.kickoff, m.id))
        assert scheduled[0].id == "esp-cpv"

    def test_after_midnight_next_match_is_belgium(self):
        """At 00:21 UTC+8 on Jun 16 (16:21 UTC on Jun 15), Spain at 00:00 UTC Jun 16 is still future.
        Actually at 00:21 UTC+8 = 16:21 UTC Jun 15, Spain kickoff is 00:00 UTC Jun 16 which is still future.
        So next_match is still Spain. We need time AFTER Spain kickoff for Belgium to be next.
        At 01:00 UTC+8 = 17:00 UTC Jun 15, Spain kickoff 00:00 UTC Jun 16 is still future.
        At 08:01 UTC+8 = 00:01 UTC Jun 16, Spain kickoff 00:00 UTC Jun 16 is in the past."""
        matches = self._make_matches()
        # After Spain match kickoff (00:00 UTC Jun 16), at 00:01 UTC Jun 16
        now_utc = datetime(2026, 6, 16, 0, 1, tzinfo=timezone.utc)
        scheduled = [m for m in matches if m.status == "scheduled" and m.kickoff > now_utc]
        scheduled.sort(key=lambda m: (m.kickoff, m.id))
        assert scheduled[0].id == "bel-egy"

    def test_match_goes_live_excluded_from_next(self):
        """When Spain match goes live, it's excluded from next_match."""
        matches = self._make_matches()
        matches[0].status = "live"  # Spain match is now live
        now_utc = datetime(2026, 6, 15, 23, 50, tzinfo=timezone.utc)
        scheduled = [m for m in matches if m.status == "scheduled" and m.kickoff > now_utc]
        scheduled.sort(key=lambda m: (m.kickoff, m.id))
        assert scheduled[0].id == "bel-egy"

    def test_match_finished_excluded_from_next(self):
        """When Spain match is finished, it's excluded from next_match."""
        matches = self._make_matches()
...
class TestBacktestRunId:
    """Test run_id-based backtest persistence."""
    ...
    def test_backtest_run_created_on_start(self):
        """BacktestRun record is created with status=running."""
        ...
        with Session() as session:
            ...
            saved = session.scalar(select(BacktestRun).where(BacktestRun.id == "test-run-001"))
            assert saved is not None
            assert saved.status == "running"
            assert saved.run_type == "standard"

    def test_backtest_run_completed(self):
        """BacktestRun can be updated to completed."""
        engine = self._make_db()
        Session = sessionmaker(bind=engine)
        with Session() as session:
            from app.models import BacktestRun
            ...
            saved = session.scalar(select(BacktestRun).where(BacktestRun.id == "test-run-002"))
            assert saved.status == "completed"
            assert saved.completed_at is not None

    def test_unique_constraint_per_run(self):
        """Duplicate (run_id, model_name, split_name, calibration_method) raises error."""
        engine = self._make_db()
        Session = sessionmaker(bind=engine)
        with Session() as session:
            from app.models import BacktestResultRecord
            r1 = BacktestResultRecord(
                run_id="run-1",
                data_version="v1",
                model_name="model-a",
                split_name="test",
                calibration_method="none",
                brier_sum=0.5, brier_mean=0.167, canonical_brier=0.5,
                log_loss=0.9, ece=0.05, top1_hit_rate=0.6, draw_recall=0.1,
                match_count=100,
            )
            session.add(r1)
            session.commit()

            r2 = BacktestResultRecord(
                run_id="run-1",
                data_version="v1",
                model_name="model-a",
                split_name="test",
                calibration_method="none",
                brier_sum=0.6, brier_mean=0.2, canonical_brier=0.6,
                log_loss=1.0, ece=0.06, top1_hit_rate=0.5, draw_recall=0.1,
                match_count=100,
            )
            session.add(r2)
    ...
    def test_different_run_id_allows_same_model_split(self):
        """Same model+split but different run_id is allowed."""
        engine = self._make_db()
        Session = sessionmaker(bind=engine)
        with Session() as session:
            from app.models import BacktestResultRecord
            ...
            session.commit()
            assert session.scalar(text("SELECT COUNT(*) FROM backtest_results")) == 2

    def test_api_reads_only_completed_runs(self):
        """API should only read from completed runs, not running/failed."""
        engine = self._make_db()
        Session = sessionmaker(bind=engine)
        with Session() as session:
            from app.models import BacktestRun, BacktestResultRecord

            # Running run with results
            run1 = BacktestRun(id="run-running", run_type="standard", status="running",
                              started_at=datetime.now(timezone.utc), code_version="v1",
                              data_version="v1", dataset_hash="h1")
            session.add(run1)
            session.flush()

            r1 = BacktestResultRecord(
                run_id="run-running", data_version="v1", model_name="model-a",
                split_name="test", calibration_method="none",
                brier_sum=0.5, brier_mean=0.167, canonical_brier=0.5,
                log_loss=0.9, ece=0.05, top1_hit_rate=0.6, draw_recall=0.1,
                match_count=100,
            )
            ...
            run2 = BacktestRun(id="run-completed", run_type="standard", status="completed",
                              started_at=datetime.now(timezone.utc), code_version="v1",
                              data_version="v1", dataset_hash="h1",
            ...
            r2 = BacktestResultRecord(
                run_id="run-completed", data_version="v1", model_name="model-a",
                split_name="test", calibration_method="none",
                brier_sum=0.6, brier_mean=0.2, canonical_brier=0.6,
                log_loss=1.0, ece=0.06, top1_hit_rate=0.5, draw_recall=0.1,
                match_count=100,
            ...
            # API logic: get latest completed run
            ...
            assert latest_completed.id == "run-completed"

            # Count results for completed run only
            count = session.scalar(
                select(BacktestResultRecord)
                .where(BacktestResultRecord.run_id == latest_completed.id)
            )
            from sqlalchemy import func
            count_val = session.scalar(
