...
def lock_due_predictions(
    ...
    """Lock the latest valid pre-match prediction for matches inside T-30.

    This is an optional helper, not the core scoring mechanism.
    The scoring system uses the latest pre-match snapshot before kickoff,
    regardless of lock status. T-30 locking is kept for backward compatibility.
    """
...
def repair_invalid_prediction_locks(session: Session) -> dict[str, int]:
    """Remove historical AI/ensemble locks created outside the T-30 window.
    ...
    for prediction in session.scalars(
        ...
        if not kickoff - timedelta(minutes=30) <= locked_at < kickoff:
...
def write_snapshots(session: Session, revision: DashboardRevision, now: datetime | None = None) -> None:
    ...
    T-30 lock is for backward compatibility only, not core scoring.
    The scoring system uses the latest pre-match snapshot before kickoff.
    """
    now = now or datetime.now(timezone.utc)
    lock_threshold = timedelta(minutes=30)
    ...
    for match in matches:
        ...
        # If it hasn't started, we write a snapshot. If within T-30, we lock it.
        ...
        is_within_t30 = now >= kickoff - lock_threshold
        ...
        if not existing_locked:
            if is_past_kickoff:
                # We missed the T-30 window and the match has kicked off.
        ...
        # If an existing_locked exists but the match hasn't kicked off yet, we still shouldn't override the lock.
        # But wait, the user said "lock at T-30". Once locked at T-30, it stays locked.
        if existing_locked and not is_past_kickoff:
            # We already have a lock, just create a normal snapshot for latest prediction
            pass
